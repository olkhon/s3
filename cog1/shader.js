...
define(["exports"], function (exports) {
	...
	function calcLightIntensity(point, normal) {
		...
		diffuse = cosAlpha < 0 ? 0 : pointLightPosition * cosAlpha;
	...
	function phongInterpolationPrepareScanline(interpolationVertexIndices, interpolationWeights, deltaX) {
		...
		for (var i = 0; i < 2; i++) {
			...
			var normalDelta = [];
			vec3.subtract(
				vertexNormals[interpolationVertexIndices[j + 1]],
				vertexNormals[interpolationVertexIndices[j]],
				normalDelta
			);
			vec3.scale(normalDelta, interpolationWeights[j + 1]);
			vec3.add(vertexNormals[interpolationVertexIndices[j]], normalDelta, scanlineInterpolationNormal)
			// Vertex interpolation.
			var vertexDelta = [];
			vec3.subtract(
				vertices(interpolationVertexIndices[j + 1]),
				vertices[interpolationVertexIndices[j]],
				vertexDelta
			);
			vec3.scale(vertexDelta, interpolationWeights[j + 1]);
			vec3.add(vertices[interpolationVertexIndices[j]], vertexDelta, scanlineInterpolationVertices);
		...
		vec3.subtract(scanlineInterpolationVertices[1], scanlineInterpolationVertices[0], scanlineInterpolationVertices[2]);
		...
		vec3.subtract(scanlineInterpolationNormals[1], scanlineInterpolationNormals[0], scanlineInterpolationNormals[2]);
