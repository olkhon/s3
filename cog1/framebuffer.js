...
define(["exports", "scene"], function (exports, scene) {
	...
	function zBufferTest(x, y, z, color) {
		...
		if (Math.abs(z - zBuf[indexZBuf]) < epsilon && color == [0, 0, 0, 255]) {
			return false;
		}
		...
		// 	// es gibt noch keinen pixel hier
		// 	zBuf[indexZBuf] = z;
		// 	return true;
		// }

		// var zBufferValue = zBuf[indexZBuf];
		// if( Math.abs(z-zBufferValue) < epsilon) {
		// 	// pixel sind gleich weit weg: zBuffer fighting
		// 	if(color == [0,0,0,255]) {
		// 		zBuf[indexZBuf] = z;
		// 		return true;
		// 	} else
